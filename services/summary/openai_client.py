...
class OpenAIClient(SummaryClient):
    ...
    def _get_summary_prompt(self) -> str:
        ...
        return """あなたは議事録要約の専門家です。
以下の議事録を300-500字の日本語で要約してください。
...
- 300-500字以内に収める"""
    ...
    def _get_partial_summary_prompt(self) -> str:
        ...
        return """以下は長い議事録の一部です。この部分を200-300字程度で要約してください。

要約の要件:
- この部分の重要なポイントを抽出する
- 後で他の部分と統合されることを前提とする
- 簡潔で要点を捉えた文章にする"""
    
    def _get_final_summary_prompt(self) -> str:
        """Get system prompt for final summarization (stage 2 of two-stage)"""
        return """以下は議事録の各部分を要約したテキストです。
これらを統合して、全体の議事録を300-500字の日本語で要約してください。

要約の要件:
- 重要な決定事項と議論のポイントを含める
- 各部分の要約を統合し、一貫性のある文章にする
- 重複を排除し、全体の流れを保つ
- 300-500字以内に収める"""